...
> Напишите перевод слова: **hello**
3. Пользователь отправляет перевод.
4. Бот отправляет правильный перевод и спрашивает пользователся правильно ли он перевел слово.
5. Пользователь отвечает да или нет.
6. Бот сохраняет ответ.
...
**parser**  
Проверяет введенный пользователем текст на корректный формат.

**notification**  
Отвечает за отправку уведомлений пользователю: получает слово для повторения, формирует сообщение и отправляет его пользователю.

**scheduler**  
Выполняет фоновые задачи по расписанию: вызывает сервис уведомлений, запускает алгоритм интервальных повторений, обновляет статистику пользователя, выбирает подходящее слово для повторения/
